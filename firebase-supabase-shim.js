// firebase-supabase-shim.v2.js — Firebase-backed shim for legacy Supabase calls
// Features: autosave to Firestore on localStorage changes, save on sign-out/tab-close,
// loadFromCloud() refreshes UI by default, debug helper __debugCloud().
(async function() {
  ...
  async function loadFromCloud(opts) {
    opts = opts || { reload:true }; // default refresh UI
    const u = auth.currentUser;
    if (!u) { if (statusEl) statusEl.textContent = "Please sign in first."; return; }
  ...
  function scheduleAutosave(delayMs) {
    ...
    saveTimer = setTimeout(() => { saveToCloud(); }, Math.max(500, delayMs||1500));
  }
  const _setItem = localStorage.setItem.bind(localStorage);
  localStorage.setItem = function(k, v) { try { _setItem(k, v); } finally { scheduleAutosave(1500); } };
  const _remove = localStorage.removeItem.bind(localStorage);
  localStorage.removeItem = function(k) { try { _remove(k); } finally { scheduleAutosave(1500); } };
  const _clear = localStorage.clear.bind(localStorage);
  localStorage.clear = function() { try { _clear(); } finally { scheduleAutosave(1500); } };
  ...
  // ---- Public API for your existing page
  window.saveToCloud = saveToCloud;
  window.loadFromCloud = function() { return loadFromCloud({ reload:true }); }; // refresh UI by default
  ...
  // ---- Supabase-shaped shim so legacy calls keep working
  const shim = {
    auth: {
      async getUser() {
        const u = auth.currentUser;
  ...
  // Wire gate buttons (as a fallback)
